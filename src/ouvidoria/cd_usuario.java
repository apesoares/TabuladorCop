...
public class cd_usuario extends javax.swing.JDialog {
    ...
    public cd_usuario(Menu mn, boolean modal) {
        super(mn, modal);
        initComponents();
    ...
    public void atualiza_painel() {
        ...
        String query = "select login as Login, nome as Nome, \n" +
                "ifnull(cop, '?') as COP, ifnull(turno, '?') as Turno, ifnull(perfil, '?') as Cod_Perfil,\n" +
                "case\n" +
                "when perfil = -1 then 'Inativo'\n" +
                "when perfil = 0 then 'DEV_MASTER'\n" +
                "when perfil = 1 then 'Supervisor'\n" +
                "when perfil = 2 then 'Representante'\n" +
                "when perfil = 4 then 'CDesk'\n" +
                
                "else '?'\n" +
                "end Perfil\n" +
                "from ouvidoria.users\n" +
        ...
        try {
            ...
            int invisible_ids[] = {4};
            int column_widths[] = {50, 150, 50, 50, 50, 150};
    ...
    private void jmni_servicos_alterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jmni_servicos_alterarActionPerformed
        ...
        if (!testaEdicao()) {
            ...
            cd_usuario_editar cad_usuario_editar = new cd_usuario_editar(
                    1,
                    main_tab.getModel().getValueAt(main_tab.getSelectedRow(), 0).toString(),
                    ...
                    main_tab.getModel().getValueAt(main_tab.getSelectedRow(), 3).toString(),
                    main_tab.getModel().getValueAt(main_tab.getSelectedRow(), 5).toString()
            );
        }
