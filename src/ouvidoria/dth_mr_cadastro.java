...
public class dth_mr_cadastro extends javax.swing.JDialog {
     ...
     */
    public dth_mr_cadastro() {
        initComponents();
    }

    public dth_mr_cadastro(Menu mn, boolean modal) {
        super(mn, modal);
        initComponents();
        ...
        global.open_modal(this, "DTH - Cadastro de microrregiões");
    ...
    public void atualiza_painel() {
        ...
        String query = "SELECT "
                + "cn.Sigla, cn.cidade_qualinet as 'Qualinet', cn.ci_depara as 'De-Para', "
                + "cn.COP, cn.Regional, cn.Cluster, cn.Sistema, cn.cop_info as 'COPInfo', count(1) as Qtd_Areas "
                + "FROM ouvidoria.cidades_novo cn "
                + "left join ouvidoria.areas_novo an on cn.cop_info = an.cidade "
                + "WHERE "
                + "("
                + "cn.sigla like '%" + jtxt_localizar.getText().trim() + "%' "
                + "or cn.cidade_qualinet like '%" + jtxt_localizar.getText().trim() + "%' "
                + "or cn.ci_depara like '%" + jtxt_localizar.getText().trim() + "%' "
                + "or cn.cop_info like '%" + jtxt_localizar.getText().trim() + "%' "
                + ") "
                + "and cn.cop = 'COP DTH' " //funcionar apenas para DTH
                + "group by cn.Sigla, cn.cidade_qualinet, cn.ci_depara, cn.COP, cn.Regional, cn.Cluster, cn.Sistema, cn.cop_info "
                + "ORDER BY cn.sigla, cn.cop_info";
        try {

            JTable tab = global.getTable(query, jpnl_planilha);

            //int invisible_ids[] = {3, 4, 5, 6};
            int invisible_ids[] = {0, 1, 4, 5, 7};
            int column_widths[] = {150, 150, 150, 150, 150, 150, 150, 150, 100};
    ...
    private void initComponents() {
        ...
        jLabel1.setText("MR/Área a localizar (parte ou todo)");
    ...
    private void jmni_servicos_alterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jmni_servicos_alterarActionPerformed
        // TODO add your handling code here:
        //global.show_message("Alterar");

        if (!testaEdicao()) {
            global.show_error_message("Nada selecionado");
        } else {
            //dth_mr_editar cad_mr_editar = new dth_mr_editar(1, main_tab.getCellEditor(main_tab.getSelectedRow(), 8).toString());
            dth_mr_editar cad_mr_editar = new dth_mr_editar(1, main_tab.getModel().getValueAt(main_tab.getSelectedRow(), 7).toString(), main_tab.getModel().getValueAt(main_tab.getSelectedRow(), 3).toString(), main_tab.getModel().getValueAt(main_tab.getSelectedRow(), 6).toString());
        }
    ...
    private void jmni_servicos_incluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jmni_servicos_incluirActionPerformed
        // TODO add your handling code here:
        //global.show_message("Incluir");

        dth_mr_editar cad_mr_editar = new dth_mr_editar(0, "", "", "");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(dth_mr_cadastro.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new dth_mr_cadastro().setVisible(true);
            }
        });
    }
